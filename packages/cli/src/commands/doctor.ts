...
export async function doctorCommand(context: CommandContext): Promise<void> {
  ...
  const report = {
    ...
    packageInstallStatus: existsSync(path.join(context.cwd, "node_modules")) ? "installed" : "missing",
    ...
    safetyPolicyStatus: existsSync(path.join(context.cwd, "docs/implementation_harness/side_effect_policy.yaml"))
      ? "present"
      : "missing"
  };
